...
def main():
    ...
    data=pd.read_csv(INpath,header=None)
    blosexp = BLOSUMAUG(data[0].values)
    peptides= pd.DataFrame(columns=["seq","seq_blosum"])
    current_original=""
    cc=0
    for index, row in blosexp.iterrows():
        if row['original sequence']!=current_original:
            cc=cc+1
            peptides.loc[cc]= [row['original sequence'], row['peptide after BLOSUM']]  
        else:
